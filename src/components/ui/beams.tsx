...
export const title = "React Beams Background"
...
// Pre-calculated animation values helper for each path
const makeAnimations = (startImmediately = false) =>
  pathData.map((_, i) => ({
    duration: 4 + (i % 5) * 0.8,
    // If starting immediately, no stagger so lines flow at once
    delay: startImmediately ? 0 : i * 0.15,
    initialProgress: (i * 5) % 100,
  }))

export const BackgroundBeams = React.memo(({ className, startImmediately = false }: BackgroundBeamsProps) => {
  const animations = makeAnimations(startImmediately)
